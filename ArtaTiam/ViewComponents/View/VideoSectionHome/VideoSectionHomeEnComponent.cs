...
{
    public class VideoSectionHomeEnComponent : ViewComponent
    {
        private Core _core = new Core();
        public async Task<IViewComponentResult> InvokeAsync()
        {
            TblConfig config = _core.Config.Get().FirstOrDefault(i => i.Key == "VideoSectionHomeTextEn");
            ...
            return await Task.FromResult((IViewComponentResult)View("~/Views/Shared/Components/VideoSectionHomeComponent" +
                "/VideoSectionHomeEnComponent.cshtml", config));
        }
    }
}